...
fun CurriculumCard(scope: CurriculumScope?, store: CurriculumSelectionStore?, manuallySelected: Boolean,
    ...
    Card(Modifier.fillMaxWidth()) { Column(Modifier.padding(16.dp), verticalArrangement = Arrangement.spacedBy(8.dp)) {
        Text("毕业规则", style = MaterialTheme.typography.titleMedium)
        if(scope != null) {
        ...
        if(summary == null) Text("毕业要求待核对，学校成绩正常保留。")
        ...
        if(scope != null) TextButton(onClick = {
            context.startActivity(Intent(Intent.ACTION_VIEW, Uri.parse(summary?.source ?: UniversityCurricula.guide(scope))))
        }) { Text("查看学校履修要项") }
        Row {
...
private fun CurriculumDialog(initial: CurriculumScope?, onDismiss: () -> Unit, onSave: (CurriculumScope?) -> Unit) {
    var faculty by remember { mutableStateOf(initial?.faculty?.takeIf { it in UniversityCurricula.departments }.orEmpty()) }
    ...
    val current = CurriculumScope(faculty, department, cohort.toIntOrNull(), program)
    AlertDialog(onDismissRequest = onDismiss, title = { Text("选择适用规则") }, text = {
        Column(Modifier.verticalScroll(rememberScrollState()), verticalArrangement = Arrangement.spacedBy(8.dp)) {
            Text("按学校的要件年度选择，不能用当前年级倒推。")
            Choice("学部／学環", faculty, UniversityCurricula.departments.keys.toList()) { faculty=it; department=""; program="" }
            Choice("学科／課程", department, UniversityCurricula.departments[faculty].orEmpty()) { department=it; program="" }
            OutlinedTextField(cohort, { if(it.length<=4 && it.all(Char::isDigit)) cohort=it }, label={ Text("规则年度") }, singleLine=true)
            Choice("コース／プログラム", program, listOf("") + UniversityCurricula.programs(current)) { program=it }
            OutlinedTextField(program, { program=it.take(120) }, label={ Text("专业方向（可补充）") })
            TextButton(onClick={ onSave(null) }) { Text("恢复学校学籍匹配") }
        }
    }, confirmButton = {
        TextButton(enabled=department in UniversityCurricula.departments[faculty].orEmpty() && current.cohort in 1900..2099,
